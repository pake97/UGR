...
from utils.computeRepairsReal import computeRepairs
...
class CGREnvironment:
    ...
    def start_fixed_arrival_simulation(self):
        ...
        for safetiness in [False]:
            ...
            #for assignment in ["random","prAsc",'prDesc','degreeDesc','degreeAsc']:
                for answer in [0,0.25,0.5,0.75,1]:
                    for users in [20]:
                        print("Running icij " +str(users) +", " +assignment +", " +str(safetiness)+", "+str(answer))        
                        self.wait_count=0
                        self.iteration_count=0
                        self.interaction_count=0
                        ...
                        compute_btw = self.neo4j_Connector.query("CALL gds.degree.write('grdg', { writeProperty: 'degree' }) YIELD centralityDistribution, nodePropertiesWritten RETURN centralityDistribution.min AS minimumScore, centralityDistribution.mean AS meanScore, nodePropertiesWritten")
                            ...
                            while graphInconsistent: 
                                ...
                                if(self.iteration_count>2*initial_violations):
                                    ...
                                    for u in self.users:
                                        ...
                                        elif assignment=="degreeDesc":
                                            assigned_hypervertex = self.neo4j_Connector.query("MATCH (v:Violation {locked : false, solved:false}) RETURN v ORDER BY v.degree DESC LIMIT 1")
                                        elif assignment=="degreeAsc":
                                            assigned_hypervertex = self.neo4j_Connector.query("MATCH (v:Violation {locked : false, solved:false}) RETURN v ORDER BY v.degree LIMIT 1")
                                        elif assignment=="prDesc":
                                            assigned_hypervertex = self.neo4j_Connector.query("MATCH (v:Violation {locked : false, solved:false}) RETURN v ORDER BY v.pageRank DESC LIMIT 1")
                                        elif assignment=="prAsc":
                                            assigned_hypervertex = self.neo4j_Connector.query("MATCH (v:Violation {locked : false, solved:false}) RETURN v ORDER BY v.pageRank LIMIT 1")
                            ...
                            while graphInconsistent: 
                                ...
                                if(self.iteration_count>2*len(violations_ids)):
                                    ...
                                    if(len(self.neo4j_Connector.query("MATCH (v:Violation {solved:false}) RETURN v"))==0):
                                        ...
                                        t2 = time.time()
                                        #compute_btw = self.neo4j_Connector.query("CALL gds.pageRank.write('grdg', { writeProperty: 'pageRank' , maxIterations: 20, dampingFactor: 0.85 }) YIELD centralityDistribution, nodePropertiesWritten RETURN centralityDistribution.min AS minimumScore, centralityDistribution.mean AS meanScore, nodePropertiesWritten,centralityDistribution.max AS maximumScore")
                                        compute_degree = self.neo4j_Connector.query("CALL gds.degree.write('grdg', { writeProperty: 'degree' }) YIELD centralityDistribution, nodePropertiesWritten RETURN centralityDistribution.min AS minimumScore, centralityDistribution.mean AS meanScore, nodePropertiesWritten")
                                        t3 = time.time()
                ...
                time2 = time.time()
                print("AVG Time elapsed: ", (time2-time1))
