...
class CGREnvironment:
    ...
    def start_fixed_arrival_simulation(self, seed):
        ...
        half_violations = int(self.violations/14)
        
        c_tuples = self.neo4j_Connector.query("MATCH (e:Comment {synthlabel:'Comment'}) return ID(e) LIMIT "+str(half_violations))
        u_tuples = self.neo4j_Connector.query("MATCH (e:User {synthlabel:'User'}) return ID(e) LIMIT "+str(int(half_violations)))
        a_tuples = self.neo4j_Connector.query("MATCH (e:Answer {synthlabel:'Answer'}) return ID(e) LIMIT "+str(int(half_violations)))
        q_tuples = self.neo4j_Connector.query("MATCH (e:Question {synthlabel:'Question'}) return ID(e) LIMIT "+str(int(half_violations)))
        t_tuples = self.neo4j_Connector.query("MATCH (e:Tag {synthlabel:'Tag'}) return ID(e) LIMIT "+str(int(half_violations)))
        
        for count, t in enumerate(c_tuples):
            if(count%3==0):
                self.neo4j_Connector.merge_query("MATCH (e:Comment) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Tag', e.injected=True")
            elif(count%3==1):
                self.neo4j_Connector.merge_query("MATCH (e:Comment) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='User', e.injected=True")
            elif(count%3==2):
                self.neo4j_Connector.merge_query("MATCH (e:Comment) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Question', e.injected=True")
        for count, t in enumerate(u_tuples):
            if(count%2==0):
                self.neo4j_Connector.merge_query("MATCH (e:User) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Answer', e.injected=True")
            else:
                self.neo4j_Connector.merge_query("MATCH (e:User) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Comment', e.injected=True")
        for count, t in enumerate(a_tuples):
            if(count%3==0):
                self.neo4j_Connector.merge_query("MATCH (e:Answer) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Question', e.injected=True")
            elif(count%3==1):
                self.neo4j_Connector.merge_query("MATCH (e:Answer) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='User', e.injected=True")
            else:
                self.neo4j_Connector.merge_query("MATCH (e:Answer) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Tag', e.injected=True")
        for count, t in enumerate(q_tuples):
            if(count%4==0):
                self.neo4j_Connector.merge_query("MATCH (e:Question) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Answer', e.injected=True")
            elif(count%4==1):
                self.neo4j_Connector.merge_query("MATCH (e:Question) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Comment', e.injected=True")
            elif(count%4==2):
                self.neo4j_Connector.merge_query("MATCH (e:Question) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Tag', e.injected=True")
            else:
                self.neo4j_Connector.merge_query("MATCH (e:Question) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='User', e.injected=True")
        for count, t in enumerate(t_tuples):
            self.neo4j_Connector.merge_query("MATCH (e:Tag) WHERE ID(e)="+str(t['ID(e)'])+" SET e.synthlabel='Question', e.injected=True")
        ...
        self.neo4j_Connector.merge_query("CALL apoc.export.csv.all('stackoverflow_neighborhood.csv', {})")
        self.neo4j_Connector.merge_query("match (n {synthlabel:'User'})-[r {deleted:False}]-(m {synthlabel:'Tag'}) MERGE (n)-[:BELONGS]->(v1:Violation {solved:false, locked:false, type:0})<-[:BELONGS]-(m)")
        self.neo4j_Connector.merge_query("match (n {synthlabel:'Answer'})-[r {deleted:False}]-(m {synthlabel:'Tag'}) MERGE (n)-[:BELONGS]->(v1:Violation {solved:false, locked:false, type:1})<-[:BELONGS]-(m)")
        self.neo4j_Connector.merge_query("match (n {synthlabel:'Comment'})-[r {deleted:False}]-(m {synthlabel:'Tag'}) MERGE (n)-[:BELONGS]->(v1:Violation {solved:false, locked:false, type:2})<-[:BELONGS]-(m)")
        self.neo4j_Connector.merge_query("match (n {synthlabel:'Comment'})-[r {deleted:False}]-(m {synthlabel:'Answer'}) MERGE (n)-[:BELONGS]->(v1:Violation {solved:false, locked:false, type:3})<-[:BELONGS]-(m)")
        self.neo4j_Connector.merge_query("match (n {synthlabel:'User'})-[r {deleted:False}]-(m {synthlabel:'User'}) MERGE (n)-[:BELONGS]->(v1:Violation {solved:false, locked:false, type:4})<-[:BELONGS]-(m)")
        self.neo4j_Connector.merge_query("match (n {synthlabel:'Answer'})-[r {deleted:False}]-(m {synthlabel:'Answer'}) MERGE (n)-[:BELONGS]->(v1:Violation {solved:false, locked:false, type:5})<-[:BELONGS]-(m)")
        self.neo4j_Connector.merge_query("match (n {synthlabel:'Question'})-[r {deleted:False}]-(m {synthlabel:'Question'}) MERGE (n)-[:BELONGS]->(v1:Violation {solved:false, locked:false, type:6})<-[:BELONGS]-(m)")
        self.neo4j_Connector.merge_query("match (n {synthlabel:'Comment'})-[r {deleted:False}]-(m {synthlabel:'Comment'}) MERGE (n)-[:BELONGS]->(v1:Violation {solved:false, locked:false, type:7})<-[:BELONGS]-(m)")
        self.neo4j_Connector.merge_query("match (n {synthlabel:'Tag'})-[r {deleted:False}]-(m {synthlabel:'Tag'}) MERGE (n)-[:BELONGS]->(v1:Violation {solved:false, locked:false, type:8})<-[:BELONGS]-(m)")
                ...
                for u in self.users:
                    ...
                    if(len(assigned_hypervertex)==0):
                        ...
                        if(violation_properties['type']==0):
                            possibilities = [('Tag', 'Tag'), ('Tag', 'Comment'), ('Tag', 'Question'), ('Tag', 'Answer'),
                                            ('User', 'User'), ('User', 'Comment'), ('User', 'Question'), ('User', 'Answer'),
                                            ('Comment', 'Tag'), ('Comment', 'User'), ('Comment', 'Comment'), ('Comment', 'Question'), ('Comment', 'Answer'),
                                            ('Question', 'Tag'), ('Question', 'User'), ('Question', 'Comment'), ('Question', 'Question'), ('Question', 'Answer'),
                                            ('Answer', 'Tag'), ('Answer', 'User'), ('Answer', 'Comment'), ('Answer', 'Question'), ('Answer', 'Answer')
                                            ]  
                            ...
                            possibilities = [('Tag', 'Tag'), ('Tag', 'User'), ('Tag', 'Comment'), ('Tag', 'Question'), 
                                            ('User', 'Tag'), ('User', 'User'), ('User', 'Comment'), ('User', 'Question'), ('User', 'Answer'),
                                            ('Comment', 'Tag'), ('Comment', 'User'), ('Comment', 'Comment'), ('Comment', 'Question'), ('Comment', 'Answer'),
                                            ('Question', 'Tag'), ('Question', 'User'), ('Question', 'Comment'), ('Question', 'Question'), ('Question', 'Answer'),
                                            ('Answer', 'User'), ('Answer', 'Comment'), ('Answer', 'Question'), ('Answer', 'Answer')
                                            ]
                            for p in possibilities:
                                possible_repair.append("MATCH (n)-[:BELONGS]->(v:Violation)<-[:BELONGS]-(m)-[r]-(n) WHERE ID(v)="+str(violation_id)+" SET v.solved=True, m.updated=True, n.updated=True, n.synthlabel='"+p[0]+"', m.synthlabel='"+p[1]+"', r.deleted=True")
                                possible_repair.append("MATCH (n)-[:BELONGS]->(v:Violation)<-[:BELONGS]-(m)-[r]-(n) WHERE ID(v)="+str(violation_id)+" SET v.solved=True, m.updated=True, n.updated=True, n.synthlabel='"+p[0]+"', m.synthlabel='"+p[1]+"'")      
                            ...
                            possibilities = [('Tag', 'Tag'), ('Tag', 'User'), ('Tag', 'Question'), ('Tag', 'Answer'),
                                            ('User', 'Tag'), ('User', 'User'), ('User', 'Comment'), ('User', 'Question'), ('User', 'Answer'),
                                            ('Comment', 'User'), ('Comment', 'Comment'), ('Comment', 'Question'), ('Comment', 'Answer'),
                                            ('Question', 'Tag'), ('Question', 'User'), ('Question', 'Comment'), ('Question', 'Question'), ('Question', 'Answer'),
                                            ('Answer', 'Tag'), ('Answer', 'User'), ('Answer', 'Comment'), ('Answer', 'Question'), ('Answer', 'Answer')
                                            ]
                            for p in possibilities:
                            ...
                            possibilities = [('Tag', 'Tag'), ('Tag', 'User'), ('Tag', 'Comment'), ('Tag', 'Question'), ('Tag', 'Answer'),
                                            ('User', 'Tag'), ('User', 'User'), ('User', 'Comment'), ('User', 'Question'), ('User', 'Answer'),
                                            ('Comment', 'Tag'), ('Comment', 'User'), ('Comment', 'Comment'), ('Comment', 'Question'),
                                            ('Question', 'Tag'), ('Question', 'User'), ('Question', 'Comment'), ('Question', 'Question'), ('Question', 'Answer'),
                                            ('Answer', 'Tag'), ('Answer', 'User'), ('Answer', 'Question'), ('Answer', 'Answer')
                                            ]
                            for p in possibilities:
                                possible_repair.append("MATCH (n)-[:BELONGS]->(v:Violation)<-[:BELONGS]-(m)-[r]-(n) WHERE ID(v)="+str(violation_id)+" SET v.solved=True, m.updated=True, n.updated=True, n.synthlabel='"+p[0]+"', m.synthlabel='"+p[1]+"', r.deleted=True")
                                possible_repair.append("MATCH (n)-[:BELONGS]->(v:Violation)<-[:BELONGS]-(m)-[r]-(n) WHERE ID(v)="+str(violation_id)+" SET v.solved=True, m.updated=True, n.updated=True, n.synthlabel='"+p[0]+"', m.synthlabel='"+p[1]+"'")      
                            ...
                            possibilities = [('Tag', 'Tag'), ('Tag', 'User'), ('Tag', 'Comment'), ('Tag', 'Question'), ('Tag', 'Answer'),
                                            ('User', 'Tag'), ('User', 'Comment'), ('User', 'Question'), ('User', 'Answer'),
                                            ('Comment', 'Tag'), ('Comment', 'User'), ('Comment', 'Comment'), ('Comment', 'Question'), ('Comment', 'Answer'),
                                            ('Question', 'Tag'), ('Question', 'User'), ('Question', 'Comment'), ('Question', 'Question'), ('Question', 'Answer'),
                                            ('Answer', 'Tag'), ('Answer', 'User'), ('Answer', 'Comment'), ('Answer', 'Question'), ('Answer', 'Answer')
                                            ]
                            ...
                            possibilities = [('Tag', 'Tag'), ('Tag', 'User'), ('Tag', 'Comment'), ('Tag', 'Question'), ('Tag', 'Answer'),
                                            ...
                                            ('Comment', 'Tag'), ('Comment', 'User'), ('Comment', 'Comment'), ('Comment', 'Question'), ('Comment', 'Answer'),
                                            ('Question', 'Tag'), ('Question', 'User'), ('Question', 'Comment'), ('Question', 'Question'), ('Question', 'Answer'),
                                            ('Answer', 'Tag'), ('Answer', 'User'), ('Answer', 'Comment'), ('Answer', 'Question')
                                            ]
                            for p in possibilities:
                                possible_repair.append("MATCH (n)-[:BELONGS]->(v:Violation)<-[:BELONGS]-(m)-[r]-(n) WHERE ID(v)="+str(violation_id)+" SET v.solved=True, m.updated=True, n.updated=True, n.synthlabel='"+p[0]+"', m.synthlabel='"+p[1]+"', r.deleted=True")
                                possible_repair.append("MATCH (n)-[:BELONGS]->(v:Violation)<-[:BELONGS]-(m)-[r]-(n) WHERE ID(v)="+str(violation_id)+" SET v.solved=True, m.updated=True, n.updated=True, n.synthlabel='"+p[0]+"', m.synthlabel='"+p[1]+"'")      
                            ...
                            possibilities = [('Tag', 'Tag'), ('Tag', 'User'), ('Tag', 'Comment'), ('Tag', 'Question'), ('Tag', 'Answer'),
                                            ('User', 'Tag'), ('User', 'User'), ('User', 'Comment'), ('User', 'Question'), ('User', 'Answer'),
                                            ('Comment', 'Tag'), ('Comment', 'User'), ('Comment', 'Comment'), ('Comment', 'Question'), ('Comment', 'Answer'),
                                            ('Question', 'Tag'), ('Question', 'User'), ('Question', 'Comment'), ('Question', 'Answer'),
                                            ('Answer', 'Tag'), ('Answer', 'User'), ('Answer', 'Comment'), ('Answer', 'Question'), ('Answer', 'Answer')
                                            ]
                            for p in possibilities:
                                possible_repair.append("MATCH (n)-[:BELONGS]->(v:Violation)<-[:BELONGS]-(m)-[r]-(n) WHERE ID(v)="+str(violation_id)+" SET v.solved=True, m.updated=True, n.updated=True, n.synthlabel='"+p[0]+"', m.synthlabel='"+p[1]+"', r.deleted=True")
                            ...
                            possibilities = [('Tag', 'User'), ('Tag', 'Comment'), ('Tag', 'Question'), ('Tag', 'Answer'),
                                            ('User', 'Tag'), ('User', 'User'), ('User', 'Comment'), ('User', 'Question'), ('User', 'Answer'),
                                            ('Comment', 'Tag'), ('Comment', 'User'), ('Comment', 'Comment'), ('Comment', 'Question'), ('Comment', 'Answer'),
                                            ('Question', 'Tag'), ('Question', 'User'), ('Question', 'Comment'), ('Question', 'Question'), ('Question', 'Answer'),
                                            ('Answer', 'Tag'), ('Answer', 'User'), ('Answer', 'Comment'), ('Answer', 'Question'), ('Answer', 'Answer')
