...
config.DATABASE_URL = "YOUR DATABASE URL"
...
class Neo4jConnector():
    ...
    def loadDatasetToNeo4j(self, dataset):
        if(dataset=='synthea'):
            try:
                injest('PATH TO /config/synthea.yml')
            except Exception as e:
                print(e)
        elif(dataset=='synthea25'):
            try:
                injest('PATH TO /config/synthea25.yml')
            except Exception as e:
                print(e)
                
        elif(dataset=='synthea50'):
            try:
                injest('PATH TO /config/synthea50.yml')
            except Exception as e:
                print(e)
        elif(dataset=='synthea75'):
            try:
                injest('PATH TO /config/synthea75.yml')
            except Exception as e:
                print(e)
        elif(dataset=='synthea100'):
            try:
                injest('PATH TO /config/synthea100.yml')
            except Exception as e:
                print(e)
        elif(dataset=='synthea125'):
            try:
                injest('PATH TO /config/synthea125.yml')
            except Exception as e:
                print(e)
        elif(dataset=='synthea150'):
            try:
                injest('PATH TO /config/synthea150.yml')
            except Exception as e:
                print(e)
        elif(dataset=='synthea200'):
            try:
                injest('PATH TO /config/synthea200.yml')
            except Exception as e:
                print(e)
        elif(dataset=='synthea500'):
            try:
                injest('PATH TO /config/synthea500.yml')
            except Exception as e:
                print(e)
        elif(dataset=='synthea1000'):
            try:
                injest('PATH TO /config/synthea1000.yml')
            except Exception as e:
                print(e)
        else:
            try:
                os.system('PATH TO /neo4j-community-5.12.0/bin/cypher-shell -u USERNAME -p PASSWORD -f ./config/'+dataset+'.cypher')
            except Exception as e:
                print(e)
