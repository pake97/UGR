...
def restoreConstraints(neo4j_Connector,n_violations):
    ...
    try:        
        ...
        t2 = time.time()
        compute_btw = neo4j_Connector.query("CALL gds.betweenness.write('grdg', { writeProperty: 'betweenness' }) YIELD centralityDistribution, nodePropertiesWritten RETURN centralityDistribution.min AS minimumScore, centralityDistribution.mean AS meanScore, nodePropertiesWritten")
        t3 = time.time()
